...
parser.add_argument(
    "--logo", "--icon", nargs=1, help="Path of the logo image.", metavar="path"
)
parser.add_argument(
    "-d",
    "--dir",
    "--path",
    nargs=1,
    help="Path where the addon is located.",
    metavar="path",
)
parser.add_argument(
    "--out",
    ...
    type=str,
    nargs="*",
    help="The output file or directory (used when creating or extracting gma files).",
)
parser.add_argument(
    "-p", "--publish", action="store_true", help="Publish the addon to the workshop."
)
parser.add_argument(
    "-v",
    "--verify",
    action="store_true",
    help="Verify the contents of the current folder and exit.",
)
parser.add_argument(
    "--new",
    "--new-addon",
    action="store_true",
    help="Create a new addon.json at the current location. This is required before an addon can be uploaded to the workshop.",
)
parser.add_argument(
    "-c",
    "--create-gma",
    action="store_true",
    help="Create a GMA file of the addon and exit.",
)
parser.add_argument(
    "--dump",
    "--dump-gma",
    action="store_true",
    help="Dump a textual representation of a gma file to console.",
)
parser.add_argument(
    "-D", "--download", action="store_true", help="Download a workshop addon."
)
parser.add_argument(
    "--workshopinfo",
    action="store_true",
    help="Get information about one or more workshop addons.",
)
parser.add_argument(
    "-x", "-e", "--extract", action="store_true", help="Extract a GMA file and exit."
)
parser.add_argument(
    "-l", "--list", action="store_true", help="List the files contained in a GMA file."
)
parser.add_argument(
    "-m",
    "--message",
    nargs=1,
    help="Update message when updating the addon.",
    metavar="msg",
)
parser.add_argument(
    "-i", "--interactive", action="store_true", help="Run GMosh interactively."
)
parser.add_argument(
    "-a",
    "--addon",
    nargs=1,
    help="Use a different addon.json file.",
    metavar="addon.json",
)
...
def main(args):
    ...
    if args.download:
        ...
        print(
            json.dumps(
                workshoputils.workshopinfo(folder_list), sort_keys=True, indent=4
            )
        )
        return
    elif args.extract:
...
def request_uploaded():
    ...
    try:
        uploaded = input(
            "No workshop ID found in the addon. Has this addon been uploaded to the workshop yet? (y/n)\n"
        )
...
def new_addon(path, addonFile="addon.json"):
    print(
        'Turning "%s" into a workshoppable addon. Press Ctrl+C at any time to cancel.'
        % path
    )
    data = dict()
        ...
        while len(data["tags"]) <= i or data["tags"][i] >= len(possible_tags):
            try:
                data["tags"].append(
                    int(
                        input(
                            "Choose tag #%s. Enter -1 to stop adding tags.\n"
                            % str(i + 1)
                        )
                    )
                )
    ...
    pop = data["tags"].pop()
    if pop != -1:
        data["tags"].append(pop)
    for i in range(0, len(data["tags"])):
        data["tags"][i] = possible_tags[data["tags"][i]]
    ...
    data["ignore"] = []
    print(
        "Now it's time to select which files you do NOT want to ship with your addon!"
    )
    print(
        "Please list the file names of the files you want to ignore in the addon. Use wildcards!"
    )
    print(
        "Checking current directory for files that are not allowed to be in a workshop addon..."
    )
    allowed, disallowed = addoninfo.GModAddon(data, path).verify_files()
    if allowed:
        print(
            "No illegal files found. But maybe you still have ideas for files to ignore?"
        )
    else:
    ...
    print(
        "Please enter the files you want to ignore. Enter nothing or -1 to continue to the next step."
    )
    while True:
        ignore = input("? ")
        if ignore == "-1" or ignore == "":
            break
    ...
    # Default changelog
    print(
        "When updating the addon on the workshop. Do you want to have a default changelog? Leave this field empty or enter -1 if you don't."
    )
    ...
    print(data)
    print(
        "Don't worry if you've made a mistake. You can either run this wizard again or edit the %s file with an editor."
        % addonFile
    )
