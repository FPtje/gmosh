...
class GModAddon:
    ...
    def has_workshop_id(self):
        ...
        return "workshopid" in self.data

    def getworkshopid(self):
        return "workshopid" in self.data and self.data["workshopid"] or 0
    ...
    def getdefault_changelog(self):
        ...
        """
        return "default_changelog" in self.data and self.data["default_changelog"] or ""
    ...
    def get_description_json(self):
        ...
        """
        a_description = (
            "description" in self.data and self.data["description"] or "Description"
        )
        a_type = self.data["type"].lower()
        a_tags = self.data["tags"]
        return json.dumps(
            {"description": a_description, "type": a_type, "tags": a_tags}
        )

    def getdescription(self):
        """Get the description of the addon"""
        return "description" in self.data and self.data["description"] or "Description"

    def gettype(self):
        """Get the type of the addon"""
        return "type" in self.data and self.data["type"] or "type"

    def gettags(self):
        """Get the tags of the addon"""
        return "tags" in self.data and self.data["tags"] or []

    def getlogo(self):
        """Get the logo of the addon"""
        return "logo" in self.data and self.data["logo"] or ""
    ...
    def getfiles(self):
        ...
        """
        ignore = [
            "*" + os.path.relpath(self.file, self.path)
        ]  # Always add the addon.json file
    ...
    def verify_files(self):
        ...
        disallowed = list(
            filter(partial(self._file_nomatch, addon_whitelist), file_list)
        )
